...
sinclude $(root_dir)/$(proj_name).mk
...
$(LIBA):
	...
	./tools/mkcreator.sh "libc/$${dir}" "$${lib_name}";make -C libc/$${dir};\
	fi;
%.d::%.S
	@echo "自动更新$*.S的依赖"
	@echo "$*.o:$*.S" > $@
...
update: #这个目标可以修复因文件变化，却没及时删除前面依赖于这个文件多出的冗余文件。
	...
	@date >> ${log_dir}/$(proj_name).log 
	@./tools/update.sh $(root_dir)/$(proj_name).mk $(root_dir) $(CPU)
ifneq "$(libc)" ""
	@make update -C libc/lib
endif
ifneq "$(OS)" ""
	@make update -C libc/os/$(OS)
endif
	@date >> ${log_dir}/$(proj_name).log
setting:
	@./tools/setting.bin
...
ifneq "$(libc)" ""
	@make clean -C libc/lib
endif
ifneq "$(OS)" ""
	@make clean -C libc/os/$(OS)
endif
	@date >> ${log_dir}/$(proj_name).log
dclean :
	${RM} $(Depend_OBJ) ${log_dir}/depend.log
ifneq "$(libc)" ""
	@make dclean -C libc/lib
endif
ifneq "$(OS)" ""
	@make dclean -C libc/os/$(OS)
endif
...
distclean:
	...
	if ! [ -z  "$(libc)" ];then \
	make distclean -C libc/lib;fi;\
	if ! [ -z "$(OS)" ];then \
	make distclean -C libc/os/$(OS);fi;\
	fi
